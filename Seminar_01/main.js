...
const reviewSymbol = Symbol('Const reviewSymbol');
const ratingSymbol = Symbol('Const ratingSymbol');
const tagsSymbol = Symbol('Const tagsSymbol');
...
class Book {
	...
	getAverageRating() {
		return (
			Math.round(
				(this.getMetadata(ratingSymbol).reduce((sum, elem) => {
					return sum + elem;
				}, 0) /
					this.getMetadata(ratingSymbol).length) *
					10
			) / 10
		);
	}
	...
	hasTag(tagName) {
		if (!this.getMetadata(tagsSymbol)) {
			return false;
		}
		return this[tagsSymbol].includes(tagName);
		// return this.getMetadata(tagsSymbol).includes(tagName);
	}
	...
	addMetadata(metadataType, data) {
		if (this[metadataType]) this[metadataType].push(data);
...
book.addMetadata(reviewSymbol, 'Книга отстой, не покупайте ее.'); // Здесь перезапись
